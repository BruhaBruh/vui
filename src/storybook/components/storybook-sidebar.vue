...
<template>
	<motion.header
		:initial="{ height: 'max-content' }" :animate="{ height: 'max-content' }"
	...
	>
		<div class="p-md border-b border-outline-variant flex items-center gap-sm">
			<RouterLink to="/" class="inline-flex flex-col flex-1">
				...
				<span class="typography-label-small text-secondary">v0.2.0</span>
			</RouterLink>
			<IconButton
				color="secondary" variant="standard" :aria-label="`Toggle theme. Current is ${mode}`"
				:icon="mode === 'dark' ? 'tabler:moon' : 'tabler:sun'" @click="next()"
			/>
		</div>
